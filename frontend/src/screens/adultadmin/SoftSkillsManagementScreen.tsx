...
const SoftSkillsManagementScreen: React.FC = () => {
  const { theme, isDarkMode } = useTheme();
  const navigation = useNavigation();
  const [modules, setModules] = useState<SoftSkillsModule[]>([]);
  const [loading, setLoading] = useState(false);
  const [refreshing, setRefreshing] = useState(false);
  const [showAddModule, setShowAddModule] = useState(false);
  const [showAddTopic, setShowAddTopic] = useState(false);
  const [selectedModule, setSelectedModule] = useState<SoftSkillsModule | null>(null);
  ...
  const loadModules = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.1.18:5000/api/adult-admin-content/soft-skills`, {
        headers
      });
      const data = await response.json();
      if (data.success && data.data && data.data.modules) {
        // Map to expected format - adult admin endpoint already filters for adults and soft skills
        const softSkillsModules = data.data.modules.map((module: any) => ({
          _id: module._id,
          title: module.title,
        ...
        console.log('=== SOFT SKILLS: Filtered modules ===', softSkillsModules);
        console.log('=== SOFT SKILLS: First module topics ===', softSkillsModules[0]?.topics);
  ...
  const handleAddModule = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        Alert.alert('Success', 'Soft Skills module created successfully');
  ...
  const handleDeleteModule = async (module: SoftSkillsModule) => {
    Alert.alert(
      'Delete Module',
    ...
    >
      <View style={styles.headerContent}>
        ...
        <View style={styles.headerInfo}>
          ...
          <Text style={styles.headerTitle}>Soft Skills Management</Text>
          <Text style={styles.headerSubtitle}>Manage communication and interpersonal skills</Text>
        </View>
        <TouchableOpacity
  ...
  const renderModuleItem = ({ item }: { item: SoftSkillsModule }) => (
        ...
        <View style={styles.topicsPreview}>
          ...
          {item.topics.slice(0, 3).map((topic, index) => (
            <View key={topic._id || index} style={styles.topicPreview}>
              ...
              >
                <MaterialIcons name="folder" size={16} color="#2196F3" />
      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.modalContent, { backgroundColor: theme.colors.surface }]}>
          <View style={styles.modalHeader}>
            <Text style={[styles.modalTitle, { color: theme.colors.text }]}>Add Soft Skills Module</Text>
          ...
          <ScrollView style={styles.modalBody}>
            <View style={styles.formGroup}>
              ...
              <TextInput
                ...
                placeholder="Enter Soft Skills module title"
                ...
                {['Beginner', 'Intermediate', 'Advanced'].map((level) => (
                  ...
                  >
                    <Text style={[
                      ...
                      { color: newModule.difficulty === level ? 'white' : '#2196F3' }
                    ]}>
                      {level}
                    </Text>
                  </TouchableOpacity>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <StatusBar barStyle={isDarkMode ? "light-content" : "dark-content"} backgroundColor="#2196F3" />
...
export default SoftSkillsManagementScreen;
