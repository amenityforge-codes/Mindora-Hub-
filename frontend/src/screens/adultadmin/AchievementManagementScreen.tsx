...
const AchievementManagementScreen: React.FC = () => {
  ...
  const loadAchievements = async () => {
    try {
      setLoading(true);
      console.log('Loading adult achievements with API service...');
      const data = await apiService.get('/adult-admin-content/achievements');
      
      console.log('Adult Achievements API response:', JSON.stringify(data, null, 2));
      
      if (data.success && data.data && data.data.achievements) {
        const achievementsList = data.data.achievements;
        setAchievements(achievementsList);
        console.log('Adult achievements loaded successfully:', achievementsList.length);
        console.log('Adult achievements data:', JSON.stringify(achievementsList, null, 2));
      } else {
        throw new Error(data.message || 'Failed to load adult achievements');
      }
    } catch (error) {
      console.error('Error loading adult achievements:', error);
      Alert.alert('Error', 'Failed to load adult achievements. Please check your connection.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateAchievement = async () => {
    try {
      ...
      const data = await apiService.post('/adult-achievements', achievementData);
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const data = await apiService.delete(`/adult-achievements/${id}`);
