...
const AdminDashboardScreen: React.FC<AdminDashboardScreenProps> = ({ navigation }) => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const response = await fetch('http://192.168.200.129:5000/api/admin/dashboard', {
        headers: {
  ...
  const loadUserProfile = async () => {
    try {
      const token = await AsyncStorage.getItem('authToken');
      if (token) {
        const response = await fetch('http://192.168.200.129:5000/api/users/profile', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
  ...
  const quickActions = [
    ...
    },
    {
      ...
      description: 'Manage modules, videos, and quizzes',
      icon: 'library-books',
      color: ['#43e97b', '#38f9d7'],
      screen: 'ModuleManagement',
    },
    {
