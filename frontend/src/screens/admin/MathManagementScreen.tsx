...
const MathManagementScreen: React.FC = () => {
  const { theme, isDarkMode } = useTheme();
  const navigation = useNavigation();
  const [modules, setModules] = useState<MathModule[]>([]);
  ...
  const loadModules = async () => {
    try {
      ...
      if (data.success) {
        // Filter for Math modules and map to expected format
        const mathModules = data.data.modules
          .filter((module: any) => module.moduleType === 'math')
          .map((module: any) => ({
            _id: module._id,
            title: module.title,
        ...
        setModules(mathModules);
      }
    } catch (error) {
      console.error('Error loading modules:', error);
      Alert.alert('Error', 'Failed to load Math modules');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddModule = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        Alert.alert('Success', 'Math module created successfully');
  ...
  const handleDeleteModule = async (module: MathModule) => {
    Alert.alert(
      'Delete Module',
    ...
    >
      <View style={styles.headerContent}>
        ...
        <View style={styles.headerInfo}>
          <MaterialIcons name="calculate" size={32} color="white" />
          <Text style={styles.headerTitle}>Math Management</Text>
          <Text style={styles.headerSubtitle}>Manage mathematical concepts and problem-solving</Text>
        </View>
        <TouchableOpacity
  ...
  const renderModuleItem = ({ item }: { item: MathModule }) => (
        ...
        <View style={styles.topicsPreview}>
          ...
          {item.topics.slice(0, 3).map((topic, index) => (
            <View key={topic._id || index} style={styles.topicPreview}>
              ...
              >
                <MaterialIcons name="folder" size={16} color="#9C27B0" />
      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.modalContent, { backgroundColor: theme.colors.surface }]}>
          <View style={styles.modalHeader}>
            <Text style={[styles.modalTitle, { color: theme.colors.text }]}>Add Math Module</Text>
          ...
          <ScrollView style={styles.modalBody}>
            <View style={styles.formGroup}>
              ...
              <TextInput
                ...
                placeholder="Enter Math module title"
                ...
                {['Beginner', 'Intermediate', 'Advanced'].map((level) => (
                  ...
                  >
                    <Text style={[
                      ...
                      { color: newModule.difficulty === level ? 'white' : '#9C27B0' }
                    ]}>
                      {level}
                    </Text>
                  </TouchableOpacity>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <StatusBar barStyle={isDarkMode ? "light-content" : "dark-content"} backgroundColor="#9C27B0" />
...
export default MathManagementScreen;
