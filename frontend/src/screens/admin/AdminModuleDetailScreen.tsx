...
const AdminModuleDetailScreen: React.FC<AdminModuleDetailScreenProps> = ({ route }) => {
  ...
  const loadUploadedVideos = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.200.129:5000/api/video/module/${moduleId}`, { 
        headers,
  ...
  const uploadVideo = async () => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.200.129:5000/api/video/upload', {
        method: 'POST',
        headers: {
                  ...
                  source={{ 
                    uri: currentVideo.videoUrl 
                      ? (currentVideo.videoUrl.startsWith('http') 
                          ...
                          : `http://192.168.200.129:5000${currentVideo.videoUrl}`)
                      : (currentVideo.thumbnail?.startsWith('http') 
                          ? currentVideo.thumbnail 
                          : `http://192.168.200.129:5000${currentVideo.thumbnail}`)
                  }}
                  useNativeControls
