...
const VideoQuizSequenceScreen: React.FC<VideoQuizSequenceScreenProps> = ({ route }) => {
  ...
  const associateQuizWithVideo = async (quiz: QuizItem) => {
    try {
      ...
      const videoResponse = await fetch(`http://192.168.200.129:5000/api/video/${selectedVideo?._id}`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({
      ...
      const quizResponse = await fetch(`http://192.168.200.129:5000/api/quiz/${quiz._id}`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({
  ...
  const removeAssociation = async (video: VideoItem) => {
    try {
      ...
      if (associatedQuiz) {
        ...
        await fetch(`http://192.168.200.129:5000/api/quiz/${associatedQuiz._id}`, {
          method: 'PUT',
          headers,
          body: JSON.stringify({
      ...
      await fetch(`http://192.168.200.129:5000/api/video/${video._id}`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({
          associatedQuiz: null
