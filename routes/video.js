...
const fileFilter = (req, file, cb) => {
  // Accept video files and image files for thumbnails
...
router.post('/upload', adminAuth, upload.fields([
  ...
  try {
    ...
    if (!title || !level || !moduleId || !req.files || !req.files.video) {
    ...
    const video = new Video({
      ...
      thumbnail: thumbnailFile ? `/uploads/videos/${thumbnailFile.filename}` : null,
...
router.put('/:id', adminAuth, async (req, res) => {
  try {
    ...
    if (associatedQuiz !== undefined) video.associatedQuiz = associatedQuiz;
    if (sequenceOrder !== undefined) video.sequenceOrder = sequenceOrder;
