...
async function saveAllLessons() {
  try {
    console.log('Saving all 20 lessons to backend...');
    
    const response = await fetch('http://192.168.200.129:5000/api/lessons/save', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ lessons: allLessons }),
    });
    ...
    if (response.ok) {
      ...
      // Verify the save
      const verifyResponse = await fetch('http://192.168.200.129:5000/api/lessons');
      if (verifyResponse.ok) {
        const savedLessons = await verifyResponse.json();
        console.log(`Verified: ${savedLessons.length} lessons saved to database`);
        console.log('Lessons saved:');
        savedLessons.forEach((lesson, index) => {
          console.log(`${index + 1}. ${lesson.title} (${lesson.topics.length} topics)`);
        });
      }
    } else {
      console.error('Failed to save lessons:', response.status, response.statusText);
    }
  } catch (error) {
